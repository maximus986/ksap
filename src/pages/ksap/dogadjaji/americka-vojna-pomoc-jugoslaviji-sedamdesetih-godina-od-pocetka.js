...
export const PAGE_QUERY = graphql`
  {
    gallery: allFile(
      filter: {
        relativeDirectory: {
          eq: "gallery/americka-vojna-pomoc-jugoslaviji-sedamdesetih-godina-od-pocetka"
        }
      }
    ) {
      edges {
    ...
    conference: contentfulHappening(
      id: { eq: "63db9cf8-1ce0-56be-8f2d-c4a049f9cea2" }
    ) {
      content {
        json
      }
...
const AmerickaVojnaPomoc = ({ data }) => {
  ...
  return (
    <Layout>
      <SEO title="HLADNORATOVSKO SAVEZNIŠTVO : Američka vojna pomoć Jugoslaviji sedamdesetih godina od početka (1951- 2021)" />
      <Hero img={childImageSharp.fluid} alt={name} hero={true}>
        <Banner>
          HLADNORATOVSKO SAVEZNIŠTVO : Američka vojna pomoć Jugoslaviji
          sedamdesetih godina od početka (1951- 2021)
        </Banner>
      </Hero>
...
export default AmerickaVojnaPomoc;
