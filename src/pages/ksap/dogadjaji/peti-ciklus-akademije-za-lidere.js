...
export const PAGE_QUERY = graphql`
  {
    gallery: allFile(
      filter: {
        relativeDirectory: { eq: "gallery/peti-ciklus-akademije-za-lidere" }
      }
    ) {
      edges {
        node {
    ...
    conference: contentfulHappening(
      id: { eq: "2975565b-752e-5278-b1bf-b7eb95826c36" }
    ) {
      content {
        json
      }
...
const PetiCiklusAkademijeZaLidere = ({ data }) => {
  ...
  return (
    <Layout>
      <SEO title="Peti ciklus Akademije za lidere" />
      <Hero img={childImageSharp.fluid} alt={name} hero={true}>
        <Banner>Peti ciklus Akademije za lidere</Banner>
      </Hero>
...
export default PetiCiklusAkademijeZaLidere;
