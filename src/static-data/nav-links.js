export default [
  {
    path: `/`,
    text: `kongres srpsko-američkog prijateljstva`,
    submenu: [
      {
        path: `/ksap/o-nama`,
        text: `o nama`,
        submenu: [
          {
            path: `/ksap/o-nama/vrednosti-i-ciljevi`,
            text: `vrednosti i ciljevi`,
          },
          {
            path: `/ksap/o-nama/osnivaci`,
            text: `osnivači`,
          },
          {
            path: `/ksap/o-nama/savetodavni-odbor`,
            text: `savetodavni odbor`,
          },
          {
            path: `/ksap/o-nama/poslovni-savet`,
            text: `poslovni savet`,
          },
          {
            path: `/ksap/o-nama/odbor-za-medije`,
            text: `odbor za medije`,
          },
          {
            text: `partneri`,
            submenu: [
              {
                path: `/ksap/o-nama/partneri/medjunarodni`,
                text: `međunarodni`,
              },
            ],
          },
        ],
      },
      {
        text: `članstvo`,
        submenu: [
          {
            path: `/ksap/clanstvo/postanite-clan`,
            text: `postanite član`,
          },
          {
            path: `/ksap/clanstvo/benefiti-clanstva`,
            text: `benefiti članstva`,
          },
          {
            path: `/ksap/clanstvo/clanovi`,
            text: `članovi`,
          },
          {
            path: `/ksap/clanstvo/sponzorstvo`,
            text: `sponzorstvo`,
          },
        ],
      },
      {
        path: `/ksap/dogadjaji`,
        text: `događaji`,
        submenu: [
          {
            path: `/ksap/dogadjaji/mogucnost-doprinosa-ksap-u-promovisanju-demokratskih-vrednosti-i-ljudskih-prava-u-republici-srbiji`,
            text: `Okrugli sto 2018`,
          },
          {
            path: `/ksap/dogadjaji/ksap-koktel`,
            text: `KSAP Koktel`,
          },
          {
            path: `/ksap/dogadjaji/srpsko-americki-odnosi-pred-izazovima-covid-19`,
            text: `Srpsko-američki odnosi pred izazovima COVID-19`,
          },
          {
            path: `/ksap/dogadjaji/pamtimo-i-stojimo-uz-sad`,
            text: `Pamtimo i stojimo uz SAD`,
          },
          {
            path: `/ksap/dogadjaji/alumni-meeting-and-networking`,
            text: `Alumni meeting and networking`,
          },
          {
            path: `/ksap/dogadjaji/treci-ciklus-srpsko-americke-akademije-za-lidere`,
            text: `Treći ciklus srpsko-američke akademije za lidere`,
          },
          {
            path: `/ksap/dogadjaji/cetvrti-ciklus-srpsko-americke-akademije-za-lidere`,
            text: `Četvrti ciklus srpsko-američke akademije za lidere`,
          },
          {
            path: `/ksap/dogadjaji/odnos-gradjana-srbije-prema-sad`,
            text: `Konferencija „Odnos građana Srbije prema SAD"`,
          },
          {
            path: `/ksap/dogadjaji/potpisivanje-sporazuma-o-saradnji-KSAP-EWB`,
            text: `Potpisivanje sporazuma o saradnji KSAP-EWB"`,
          },
          {
            path: `/ksap/dogadjaji/metju-palmer-na-konferenciji-KSAP-i-EWB`,
            text: `Metju Palmer na konferenciji KSAP-a i EWB-a"`,
          },
        ],
      },
      {
        path: `/ksap/aktuelnosti`,
        text: `aktuelnosti`,
      },
      {
        path: `/ksap/kontakt`,
        text: `kontakt`,
      },
    ],
  },
  {
    text: `centar za američke studije vudro vilson`,
    submenu: [
      {
        path: `/centar-za-americke-studije-vudro-vilson/o-centru`,
        text: `o centru`,
      },
      {
        path: `/centar-za-americke-studije-vudro-vilson/srpsko-americka-akademija-za-lidere`,
        text: `srpsko-američka akademija za lidere`,
        submenu: [
          {
            path: `/centar-za-americke-studije-vudro-vilson/srpsko-americka-akademija-za-lidere/polaznici`,
            text: `polaznici`,
          },
          {
            path: `/centar-za-americke-studije-vudro-vilson/srpsko-americka-akademija-za-lidere/prijava-i-selekcija-kandidata`,
            text: `prijava i selekcija kandidata`,
          },
          {
            path: `/centar-za-americke-studije-vudro-vilson/srpsko-americka-akademija-za-lidere/moduli`,
            text: `moduli`,
          },
        ],
      },
      {
        path: `/centar-za-americke-studije-vudro-vilson/analize-i-istrazivanja`,
        text: `analize i istraživanja`,
      },
      {
        path: `/centar-za-americke-studije-vudro-vilson/predavaci`,
        text: `predavači`,
      },
    ],
  },
  {
    text: `centar za regionalnu bezbednost`,
    submenu: [
      {
        path: `/centar-za-regionalnu-bezbednost-i-transatlantske-integracije-ronald-regan/o-centru/`,
        text: `o centru`,
      },
      {
        text: `akademija`,
        submenu: [
          {
            path: `/centar-za-regionalnu-bezbednost-i-transatlantske-integracije-ronald-regan/akademija/akademija`,
            text: `akademija`,
          },
          {
            path: `/centar-za-regionalnu-bezbednost-i-transatlantske-integracije-ronald-regan/akademija/polaznici`,
            text: `polaznici`,
          },
          {
            path: `/centar-za-regionalnu-bezbednost-i-transatlantske-integracije-ronald-regan/akademija/moduli`,
            text: `moduli`,
          },
          {
            path: `/centar-za-regionalnu-bezbednost-i-transatlantske-integracije-ronald-regan/akademija/predavaci`,
            text: `predavači`,
          },
        ],
      },
      {
        path: `/centar-za-regionalnu-bezbednost-i-transatlantske-integracije-ronald-regan/projekti`,
        text: `projekti`,
      },
    ],
  },
];
